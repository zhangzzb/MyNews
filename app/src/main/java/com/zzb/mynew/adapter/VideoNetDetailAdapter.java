...
public class VideoNetDetailAdapter extends ListBaseAdapter<VideoBean> {
    public VideoNetDetailAdapter(Context context){
        super(context);
    }
    ...
    protected View getRealView(int position, View convertView, ViewGroup parent) {
        ...
        VideoBean items = mDatas.get(position);
        if (items != null) {
                //用户名
                holder.mTvVideoDetailsName.setText(items.getTopicName());
                //用户头像
                ImageLoaderUtils.displayRound(parent.getContext(),holder.mCivUserDetail,items.getTopicImg());
                //标题
                holder.mTvVideoDetailsTitle.setText(items.getTitle());
                //视频
                holder.mJcVideoPlayer.setUp(items.getMp4_url(),JCVideoPlayerStandard.SCREEN_LAYOUT_NORMAL,"");
                //加载显示图
                ImageLoaderUtils.display(parent.getContext(), holder.mJcVideoPlayer.thumbImageView,items.getCover());
